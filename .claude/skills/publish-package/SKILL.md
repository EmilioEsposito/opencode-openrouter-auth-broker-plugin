...
```bash
npm view opencode-openrouter-auth-broker-plugin version
gh run list --repo EmilioEsposito/opencode-openrouter-auth-broker-plugin --workflow publish.yml --limit 5
```

For a specific run:

```bash
gh run view --repo EmilioEsposito/opencode-openrouter-auth-broker-plugin <run-id>
```
